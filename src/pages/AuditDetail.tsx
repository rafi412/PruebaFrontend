...
import { ArrowLeft, Play, CheckCircle2, Circle, AlertTriangle, RefreshCw } from 'lucide-react';
...
export default function AuditDetail() {
  ...
  if (!initialData) return <div className="p-10 text-gray-500 font-medium">Cargando detalles de auditoría...</div>;

  return <AuditDetailContent auditData={initialData} onBack={() => navigate('/')} />;
}

/** 
 * Componente interno para gestionar la lógica y visualización del detalle 
 */
function AuditDetailContent({ auditData, onBack }: { auditData: Audit, onBack: () => void }) {
  // Integración del motor de simulación
  const { audit, isRunning, runSimulation } = useAuditRunner(auditData);

  // Cálculo de estado derivado para identificar incidencias
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Listado detallado de puntos de control */}
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
        ...
        <div className="divide-y divide-gray-100">
          {audit.checks.map((check) => (
            <div key={check.id} className="px-6 py-4 flex items-center justify-between hover:bg-gray-50/50 transition-colors">
              <div className="flex items-center gap-4">
                <CheckStatusIcon status={check.status} />
                <div>
                  <p className="font-semibold text-gray-900">{check.title}</p>
                  <p className="text-[10px] text-gray-400 uppercase tracking-widest font-black">Prioridad {check.priority}</p>
                </div>
              </div>
              <span className={cn(
...
/** 
 * Renderiza el icono correspondiente según el estado técnico del check 
 */
function CheckStatusIcon({ status }: { status: string }) {
  if (status === 'OK') return <CheckCircle2 className="text-green-500" size={22} />;
  if (status === 'KO') return <AlertTriangle className="text-red-500" size={22} />;
  if (status === 'RUNNING') return <div className="w-5 h-5 border-3 border-blue-500 border-t-transparent rounded-full animate-spin" />;
  return <Circle className="text-gray-300" size={22} />;
}